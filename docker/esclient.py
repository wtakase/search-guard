...
r = http.request('GET', 'https://sgssl-0.example.com:9200', headers=headers)
udata=r.data
print(udata)
...
sock.settimeout(10)
wrappedSocket = ssl.wrap_socket(sock=sock, cert_reqs=ssl.CERT_REQUIRED, keyfile='/usr/share/elasticsearch/config/CN=kirk,OU=client,O=client,L=Test,C=DE.key.pem', certfile='/usr/share/elasticsearch/config/CN=kirk,OU=client,O=client,L=Test,C=DE.crtfull.pem',  ca_certs='/usr/share/elasticsearch/config/chain-ca.pem', ssl_version=ssl.PROTOCOL_TLSv1_2)
